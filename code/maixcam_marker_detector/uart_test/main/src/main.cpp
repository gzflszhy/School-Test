...
// TX: A19 (UART1_TX), device: /dev/ttyS1, 115200 8N1, 20 Hz.
...
namespace {
constexpr std::uint32_t kMagic = 0x31435641U;  // "AVC1" on the wire (little-endian)
...
int main() {
    ...
    std::printf("AVC1 UART test started: A19 UART1_TX, /dev/ttyS1, 115200 8N1, 20 Hz\n");

    while (true) {
        // 15-second cycle: 0.00 -> +0.05 -> 0.00 m/s.
        const std::uint64_t phase =
            (maix::time::ticks_ms() / 1000U - t0) % 15U;
        ...
        std::printf("sent seq=%u vx=%.2f valid=1 (%d bytes)\n",
                    seq - 1U, vx, written);
        usleep(50000);  // 20 Hz
    }
