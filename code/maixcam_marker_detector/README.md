...
当前主判定只依赖三个 30 mm 大 L：三者分别通过轮廓形状筛选、组成符合 CAD 比例的直角三角形，并在仿射归一化后覆盖三个大 L 模板，即可输出 `TRACKABLE`。右上角的小 L、两个方块和黑色板不再是主检测成功的硬条件；它们只能增加报告置信度，两个以上小图形命中时可进一步输出方向和 `FULL_ID`。
