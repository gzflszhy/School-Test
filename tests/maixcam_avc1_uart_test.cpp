// maixcam_avc1_uart_test.cpp — standalone UART TX test for MaixCAM Pro
...
int main() {
    ...
    std::uint32_t seq = 0;
    const std::uint64_t t0 = maix::time::ticks_ms() / 1000ULL;

    while (true) {
        // 15 s cycle: 0 -> +0.05 -> 0, at 20 Hz.
        const std::uint64_t phase =
            (maix::time::ticks_ms() / 1000ULL - t0) % 15ULL;
        const float vx = (phase >= 5ULL && phase < 10ULL) ? 0.05F : 0.0F;
        const float vy = 0.0F;
        ...
        int written = 0;
        while (written < 24) {
            const int n = uart.write(frame + written, 24 - written);
            if (n <= 0) {
        ...
        std::printf("sent seq=%u vx=%.2f (%d bytes)\n", seq - 1, vx, written);
        usleep(50000);  // 20 Hz
    }
